...
AND 
	department IN ('First Aid', 'Computers', 'Movies');
...
--If we try to filter using the where clause as shown below, it will throw an error.
--This is because the window clause gets executed after the where clause. It will think
--that the rank column does not exist.
SELECT 
	first_name
	, email
	, department
...
--Instead use a subquery and filter the outer query
SELECT 
	* 
FROM
	(
...
-- Create three groups from NTILE()in the query above
...
--The above is very expensive and inefficient. It takes (here) 344 ms. Instead use 
-- the query below. Here it only took 144 ms. Much more efficient and yields same results.
...
FROM
	employees;
--The above uses ORDER BY combined with window function. This results in a running
--total. It can be partitioned by any other columns.
...
SELECT 
	...
	,SUM(salary)
		OVER(
			...
			ORDER BY
				hire_date
			) AS salary_running_total_by_dept
FROM employees;
...
-- number of employees working in that department and in are located in region 2
SELECT 
	first_name
	,department
	,COUNT(*) 
